...
int _mmradio_mute(mm_radio_t *radio)
{
	...
	if (radio->radio_fd < 0) {
		...
		MMRADIO_LOG_WARNING("radio_fd (%d) is not opened", radio->radio_fd);
		return MM_ERROR_NONE;
	}

	/*Why mute if already muted*/
	if (radio->is_muted) {
		MMRADIO_LOG_WARNING("Already Muted why mute again");
		return MM_ERROR_NONE;
	}
	...
	if (radio->radio_fd > 0) {
		(priv_sprd_v4l2->vctrl).id = V4L2_CID_AUDIO_MUTE;
		(priv_sprd_v4l2->vctrl).value = 1; /* mute */

		if (ioctl(radio->radio_fd, VIDIOC_S_CTRL, &(priv_sprd_v4l2->vctrl)) < 0) {
			MMRADIO_LOG_ERROR(" failed VIDIOC_S_CTRL- %s\n", strerror(errno));
			pthread_mutex_unlock(&radio->volume_lock);
			return MM_ERROR_RADIO_NOT_INITIALIZED;
		}
	}
...
int _mmradio_unmute(mm_radio_t *radio)
{
	...
	/*Why ummute if already ummuted*/
	if (!radio->is_muted) {
		MMRADIO_LOG_WARNING("Already Unmuted why unmute again");
		return MM_ERROR_NONE;
	}
	...
	if (radio->radio_fd > 0) {
		(priv_sprd_v4l2->vctrl).id = V4L2_CID_AUDIO_MUTE;
		(priv_sprd_v4l2->vctrl).value = 0; /* unmute */
...
int _mmradio_start(mm_radio_t *radio)
{
	...
	/* unmute : mute is not called but broadcom start as muted. we will unmute at here */
	if (radio->is_muted == MM_RADIO_FALSE) {
		MMRADIO_LOG_INFO("unmute in start sequence");
		if (__mmradio_unmute_internal(radio) != MM_ERROR_NONE) {
			MMRADIO_LOG_ERROR("set unmute_internal is failed\n");
		}
	}
